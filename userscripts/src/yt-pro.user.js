...
// @version      4.2.0
// @description  Unified YouTube optimizer: CPU/GPU/UI optimizations + automatic quality control + premium playback + comprehensive experiment flags
// @author       Ven0m0 (optimizer), adisib/Fznhq (quality control), CY Fung (flags)
...
(()=>{
  ...
  const CONSTANTS={
    RAF_BASE_ID:1e9,RAF_BATCH_SIZE:10,INDEXEDDB_CLEANUP_DELAY:2e4,PREFETCH_TIMEOUT:3e4,
    TIMER_CHECK_INTERVAL:10,TIMER_REPATCH_DELAY:800,TIMER_NAVIGATE_DEBOUNCE:500,
    IDLE_CHECK_INTERVAL:2e3,IDLE_THROTTLE:100,IDLE_MIN_DELAY_ACTIVE:150,ACTIVITY_THROTTLE:100,
    FLAG_UPDATE_THROTTLE:1e3,VISIBILITY_THROTTLE:1e3,LAZY_LOAD_THROTTLE:500,LAZY_LOAD_INITIAL_DELAY:100,
    AMBIENT_CHECK_DELAY:500,AMBIENT_THROTTLE:1e3,QUALITY_INIT_DELAY:100,QUALITY_RETRY_DELAY:100,
    QUALITY_STORAGE_EXPIRY:2592e6,INSTANT_NAV_THROTTLE:200,LAZY_THUMB_ROOT_MARGIN:"1000px",
    SCROLL_DEBOUNCE:60,WHEEL_DEBOUNCE:60,RESIZE_DEBOUNCE:120
  };

  const CFG={
    debug:0,
    cpu:{
      eventThrottle:1,rafDecimation:1,timerPatch:1,idleBoost:1,
      idleDelayNormal:8e3,idleDelayShorts:15e3,rafFpsVisible:20,rafFpsHidden:3,
      minDelayIdle:200,minDelayBase:75
    },
    ...
    quality:{
      enabled:1,targetRes:"hd1080",highFramerateTargetRes:null,preferPremium:0,
      flushBuffer:1,useAPI:1,overwriteStoredSettings:0
    },
    flags:{
      IS_TABLET:1,
      ...
      web_animated_like:0,
      web_animated_like_lazy_load:0,
      web_animated_actions_v2:0,
      render_unicode_emojis_as_small_images:1,
      smartimation_background:0,
      kevlar_refresh_on_theme_change:0,
      kevlar_measure_ambient_mode_idle:0,
      kevlar_watch_cinematics_invisible:0,
      ...
      web_watch_cinematics_preferred_reduced_motion_default_disabled:0,
      kevlar_tuner_should_reuse_components:1,
      kevlar_tuner_should_test_reuse_components:1,
      kevlar_should_reuse_components:1,
      kevlar_should_maintain_stable_list:1,
      kevlar_tuner_default_comments_delay:0,
      kevlar_tuner_run_default_comments_delay:0,
      ...
      enable_player_logging_lr_home_infeed_ads:0,
      log_gel_compression_latency:1,
      log_gel_compression_latency_lr:1,
      log_jspb_serialize_latency:1,
      kevlar_watch_metadata_refresh_standardized_body_typography:0,
      kevlar_refresh_gesture:0,
      use_cfr_monitor:0,
      skip_network_check_if_cfr:0,
      kevlar_watch_grid:0,
      kevlar_watch_grid_hide_chips:0,
      kevlar_watch_grid_reduced_top_margin_rich_grid:0,
      kevlar_watch_grid_top_companion:0,
      kevlar_watch_fixie:0,
      kevlar_watch_grid_auto_open_playlist:0,
      action_companion_center_align_description:0,
      action_companion_truncate_domain:0,
      ...
      enable_shadydom_free_scoped_node_methods:0,
      enable_shadydom_free_scoped_query_methods:0,
      enable_shadydom_free_scoped_readonly_properties_batch_one:0,
      enable_shadydom_free_parent_node:0,
      enable_shadydom_free_children:0,
      enable_shadydom_free_last_child:0,
      debug_forward_web_query_parameters:0,
      debug_handle_relative_url_for_query_forward_killswitch:0,
      debug_web_ve_data:0
    }
  };
  ...
  const IDLE_ATTR="data-yt-idle",CV_OFF_ATTR="data-yt-cv-off";
  ...
  const rafThrottle=fn=>{
    let q=0;
    return function(...a){
      if(!q){q=1;requestAnimationFrame(()=>{fn.apply(this,a);q=0;});}
    };
  };

  const GM=window.GM||{getValue:GM_getValue,setValue:GM_setValue};
  const getStoredValue=async(key,def)=>{
    try{if(GM.getValue)return await GM.getValue(`yt_opt_${key}`,def);return def;}catch{return def;}
  };
  const setStoredValue=async(key,val)=>{
    try{if(GM.setValue)await GM.setValue(`yt_opt_${key}`,val);}catch{}
  };

  let recentVideo="",foundHFR=0;
  ...
  (()=>{
    ...
    if(win?.indexedDB?.constructor?.name==="IDBFactory"){
      ...
      let cleanupTimer=0;
      const cleanup=()=>{
        for(const req of openDBs){try{req.result?.close();}catch{}}
        openDBs.clear();
        for(const[db]of closedDBs){try{db?.close();}catch{}}
        closedDBs.clear();
      };
  ...
  if(CFG.gpu.blockAV1){
    ...
    if(navigator.mediaCapabilities?.decodingInfo){
      ...
      navigator.mediaCapabilities.decodingInfo=async config=>{
        if(/av01/i.test(config?.video?.contentType||""))return{supported:0,powerEfficient:0,smooth:0};
  ...
  (()=>{
    ...
    if(CFG.ui.contentVisibility)css+=`html:not([${CV_OFF_ATTR}]) #comments,html:not([${CV_OFF_ATTR}]) #related,html:not([${CV_OFF_ATTR}]) ytd-watch-next-secondary-results-renderer{content-visibility:auto!important;contain-intrinsic-size:auto 1px 1000px}`;
    ...
    if(CFG.gpu.disableAmbient)css+=".ytp-ambient-light,ytd-watch-flexy[ambient-mode-enabled] .ytp-ambient-light{display:none!important}ytd-app,ytd-watch-flexy,#content,#page-manager{backdrop-filter:none!important;filter:none!important}";
  ...
  if(CFG.cpu.eventThrottle){
    ...
    const debounceEvents=new Map([["scroll",CONSTANTS.SCROLL_DEBOUNCE],["wheel",CONSTANTS.WHEEL_DEBOUNCE],["resize",CONSTANTS.RESIZE_DEBOUNCE]]);
    ...
    EventTarget.prototype.addEventListener=function(type,fn,opt){
      if(isShorts()||!CFG.cpu.eventThrottle||typeof fn!=="function"||isPlayer(this)||(isGlobal(this)&&(type==="wheel"||type==="scroll"||type==="resize")))return origAdd.call(this,type,fn,opt);
      let wrapped=fn;
  ...
  if(CFG.cpu.rafDecimation){
    ...
    let idc=1;
    const rafQ=new Map();
    let rafSch=0,nextFrm=performance.now();
    ...
    const processQueue=()=>{
      ...
      if(now>=nextFrm){
        ...
        for(let i=0;i<bs;i++){try{cbs[i](now);}catch{}}
        for(let i=bs;i<cbs.length;i++)origRAF(()=>{try{cbs[i](now);}catch{}});
      }
    ...
    window.cancelAnimationFrame=id=>{
      typeof id==="number"&&id>=CONSTANTS.RAF_BASE_ID?rafQ.delete(id):origCAF(id);
    };
    document.addEventListener("visibilitychange",throttle(()=>{nextFrm=performance.now();},CONSTANTS.VISIBILITY_THROTTLE));
  ...
  (async()=>{
    ...
    if(!document.documentElement)await new Promise(r=>{
    ...
    if(document.visibilityState==="visible"){
      ...
      await new Promise(r=>{
        const check=()=>{iframe.contentWindow?.setTimeout?r():setTimeout(check,CONSTANTS.TIMER_CHECK_INTERVAL);};
        check();
      });
      iframeTimers={
    ...
    const scheduleCallback=cb=>{
      if(document.visibilityState==="visible")return new Promise(r=>{
        ...
        ob.observe(trigger,{attributes:true});
        trigger.setAttribute("data-trigger",Math.random().toString(36).slice(2));
      }).then(cb);
      return new Promise(requestAnimationFrame).then(cb);
    };
    const wrapTimeout=(impl,tracked)=>(fn,delay=0,...a)=>{
      if(typeof fn!=="function"||isShorts()||!throttleTimers||delay<minDelay)return natv.setTimeout(fn,delay,...a);
      const exec=()=>fn.apply(window,a);
      const id=impl(()=>scheduleCallback(exec),delay);
      tracked.add(id);
      return id;
    };
    const wrapClear=tracked=>id=>{
      if(tracked.has(id)){tracked.delete(id);iframeTimers.clearTimeout(id);}else natv.clearTimeout(id);
    };
    const wrapInterval=impl=>(fn,delay=0,...a)=>{
      if(isShorts()||typeof fn!=="function"||delay<minDelay||!throttleTimers)return natv.setInterval(fn,delay,...a);
    ...
    if(CFG.cpu.idleBoost){
      ...
      const thAct=throttle(()=>{
        ...
        if(document.documentElement.hasAttribute(IDLE_ATTR)){
          ...
          throttleTimers=1;minDelay=CFG.cpu.minDelayBase;log("Idle OFF");
        }
    ...
    const throttledNavigate=debounce(()=>{unpatchTimers();setTimeout(patchTimers,CONSTANTS.TIMER_REPATCH_DELAY);},CONSTANTS.TIMER_NAVIGATE_DEBOUNCE);
  ...
  if(CFG.ui.instantNav){
    const throttledMouseover=throttle(e=>{
      ...
      if(link){
        ...
        setTimeout(()=>{prefetch.parentNode?.removeChild(prefetch);},CONSTANTS.PREFETCH_TIMEOUT);
      }
  ...
  if(CFG.gpu.lazyThumbs){
    ...
    const lazyLoad=()=>{
      const el=document.querySelectorAll("ytd-rich-item-renderer:not([data-lazy-opt]),ytd-compact-video-renderer:not([data-lazy-opt]),ytd-thumbnail:not([data-lazy-opt])");
      el.forEach(e=>{e.dataset.lazyOpt="1";e.style.display="none";thumbObserver.observe(e);});
    };
    ...
    else setTimeout(lazyLoad,CONSTANTS.LAZY_LOAD_INITIAL_DELAY);
  }
  ...
  if(CFG.gpu.disableAmbient){
    const disableAmbient=()=>{
      ...
      const ambientObs=new MutationObserver(mutations=>{
        mutations.forEach(m=>{
          if(m.type==="attributes"&&m.attributeName==="ambient-mode-enabled"&&flexy.hasAttribute("ambient-mode-enabled"))flexy.removeAttribute("ambient-mode-enabled");
        });
      });
    ...
    else setTimeout(disableAmbient,CONSTANTS.AMBIENT_CHECK_DELAY);
  ...
  if(CFG.quality.enabled){
    ...
    const getVideoIDFromURL=ytPlayer=>{
      const idMatch=/(?:v=)([\w-]+)/,matches=idMatch.exec(ytPlayer.getVideoUrl());
    ...
    const setResolution=(ytPlayer,resolutions)=>{
      ...
      const shouldPremium=CFG.quality.preferPremium&&[...ytPlayer.getAvailableQualityData()].some(q=>q.quality===res&&q.qualityLabel.includes("Premium")&&q.isPlayable);
      ...
      if(resolutions.indexOf(res)<resolutions.indexOf(currentQuality)){
        ...
        while(ytResolutions.indexOf(resolutions[nextBestIndex])===-1&&nextBestIndex<end)++nextBestIndex;
        if(!useButtons&&CFG.quality.flushBuffer&&currentQuality!==resolutions[nextBestIndex]){
      ...
      if(useButtons){
        if(shouldPremium){
          const premiumData=[...ytPlayer.getAvailableQualityData()].find(q=>q.quality===res&&q.qualityLabel.includes("Premium"));
        ...
        try{
          ...
          if(settingsButton){
            ...
            const qualityMenuButton=document.evaluate('.//*[contains(text(),"Quality")]/ancestor-or-self::*[@class="ytp-menuitem-label"]',ytPlayer,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue;
            if(qualityMenuButton){
              unwrapElement(qualityMenuButton).click();
              const qualityButton=document.evaluate('.//*[contains(text(),"'+HEIGHTS[resolutions.indexOf(res)]+'") and not(@class)]/ancestor::*[@class="ytp-menuitem"]',ytPlayer,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue;
              if(qualityButton){unwrapElement(qualityButton).click();log("Quality (Buttons):",res);}
            }
          }
        }catch(e){log("Quality button error:",e);}
      }
    };
    const setResOnReady=(ytPlayer,resolutions)=>{
      if(CFG.quality.useAPI&&ytPlayer.getPlaybackQuality===undefined)window.setTimeout(setResOnReady,CONSTANTS.QUALITY_RETRY_DELAY,ytPlayer,resolutions);
      else{
        let framerateUpdate=0;
        if(CFG.quality.highFramerateTargetRes){
          const features=ytPlayer.getVideoData().video_quality_features;
          if(features){const isHFR=features.includes("hfr");framerateUpdate=isHFR&&!foundHFR;foundHFR=isHFR;}
        }
        ...
        if(curVid!==recentVideo||framerateUpdate){
          ...
          if(!storedQuality||storedQuality.indexOf(CFG.quality.targetRes)===-1){
            const tc=Date.now(),te=tc+CONSTANTS.QUALITY_STORAGE_EXPIRY;
    ...
    window.addEventListener("loadstart",e=>{
      ...
      const ytPlayer=document.getElementById("movie_player")||document.getElementsByClassName("html5-video-player")[0];
      const ytPlayerUnwrapped=unwrapElement(ytPlayer);
      if(ytPlayerUnwrapped){log("Loaded new video");setResOnReady(ytPlayerUnwrapped,RESOLUTIONS);}
    },true);
    ...
    else setTimeout(initQuality,CONSTANTS.QUALITY_INIT_DELAY);
  }
  ...
  log("YouTube Unified Optimizer v4.2.0 loaded");
})();
