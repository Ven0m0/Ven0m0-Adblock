...
(() => {
	...
	(() => {
		...
		if (CFG.gpu.disableAmbient)
			css +=
				".ytp-ambient-light,ytd-watch-flexy[ambient-mode-enabled] .ytp-ambient-light{display:none!important}ytd-app,ytd-watch-flexy,#content,#page-manager{backdrop-filter:none!important;filter:none!important;animation:none!important;will-change:auto!important}";
	...
	(async () => {
		...
		if (!document.documentElement)
			await new Promise((r) => {
				...
				else document.addEventListener("DOMContentLoaded", r, { once: !0 });
			});
		...
		if (document.visibilityState === "visible") {
			...
			iframeTimers = {
				...
				clearTimeout: iframe.contentWindow.clearTimeout.bind(
					iframe.contentWindow,
				),
				setInterval: iframe.contentWindow.setInterval.bind(
					iframe.contentWindow,
				),
		...
		if (CFG.cpu.idleBoost) {
			...
			activityEv.forEach((evt) =>
				window.addEventListener(evt, thAct, { capture: !0, passive: !0 }),
			);
			setInterval(() => {
				...
				if (now - lastActive >= idl) {
					if (!document.documentElement.hasAttribute(IDLE_ATTR)) {
						...
						const hv =
							document.querySelector("video.video-stream")?.paused === !1;
	...
	if (CFG.gpu.lazyThumbs) {
		...
		if (document.body)
			lazyObs.observe(document.body, { childList: !0, subtree: !0 });
	...
	if (CFG.quality.enabled) {
		const unwrapElement = (el) =>
			el?.wrappedJSObject ? el.wrappedJSObject : el;
		...
		const setResolution = (ytPlayer, resolutions) => {
			...
			if (useButtons) {
				if (shouldPremium) {
					...
					if (premiumData)
						log("Premium quality available:", premiumData.qualityLabel);
				}
				try {
		...
		const setResOnReady = (ytPlayer, resolutions) => {
			if (CFG.quality.useAPI && ytPlayer.getPlaybackQuality === undefined) {
				...
				if (curVid !== recentVideo || framerateUpdate) {
					...
					if (
						!storedQuality ||
						storedQuality.indexOf(CFG.quality.targetRes) === -1
					) {
						const tc = Date.now(),
							te = tc + 2592000000;
