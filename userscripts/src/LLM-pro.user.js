...
(() => {
	...
	if (isCGPT) {
		const INIT_MSG = 10,
			MAX_ITER = 800,
			rx =
				/^https:\/\/chatgpt\.com\/backend-api\/conversation\/[a-f0-9-]{36}$/i;
	...
	if (isCGPT) {
		...
		const getTx = () =>
			txCache || (txCache = document.querySelector("form textarea"));
		const getStopBtn = () =>
			stopBtnCache ||
			(stopBtnCache = document.querySelector(
				'button[data-testid$="stop-button"]',
			));
		const getSubmit = () =>
			submitCache ||
			(submitCache = document.querySelector(
