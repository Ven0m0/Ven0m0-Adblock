...
(() => {
  ...
  const C = {
    ...
    SCRIPT_DENY: /ads?|analytics|tracking|doubleclick|googletag|gtag|google-analytics|adsbygoogle|consent|pixel|facebook|scorecardresearch|matomo|tealium|pardot|hubspot|hotjar|intercom|criteo|quantc/i,
    IO_MARGIN: "300px",
    BATCH: 30
  };
  const TRACK = ["fbclid", "gclid", "utm_source", "utm_medium", "utm_campaign", "utm_content", "utm_term", "utm_id", "mc_cid", "mc_eid", "_ga", "pk_campaign", "scid", "src", "ref", "aff", "affiliate", "campaign", "ad_id", "ad_name", "tracking", "partner", "promo", "promoid", "clickid", "irclickid", "spm", "smid", "pvid", "qid", "traffic_source", "sprefix", "rowan_id1", "rowan_msg_id"];
  const HASH = ["intcid", "back-url", "back_url", "src"];
  const UE = ["click", "keydown", "touchstart", "pointerdown"];
  const DEF = { log: 0, lazy: 1, iframes: 1, videos: 1, defer: 1, observe: 1, prefetch: 1, preconnect: 1, linkPrefetch: 1, linkLimit: 10, linkDelay: 3000, gpu: 1, mem: 1, preload: 1, cleanURL: 1, bypass: 1, rightClick: 0, copy: 1, select: 1, cookie: 1, tabSave: 1, cpuTamer: 1, rafTamer: 1, caching: 1, minTimeout: C.TIME.MIN_TO, minInterval: C.TIME.MIN_IV, showUI: 1 };
  const cfg = (() => {
    try {
  ...
  const state = { cache: new Map(), cacheSize: 0, loaded: new WeakSet(), deferredScripts: new Map(), origins: new Set(), interactionBound: 0 };
  const idle = (fn, to = C.TIME.IDLE) => ("requestIdleCallback" in window ? requestIdleCallback(fn, { timeout: to }) : setTimeout(fn, C.TIME.FALLBACK));
  ...
  if (cfg.cpuTamer || cfg.rafTamer) {
    ...
    if (cfg.cpuTamer) {
      window.setTimeout = (fn, d = 0, ...a) => {
        let id;
        const w = typeof fn === "function" ? (...x) => awaitTO(id).then((v) => v && fn(...x)).catch(throwE) : fn;
  ...
  function canonicalAmazon(url) {
    ...
    const asin = (p.match(/\/dp\/([A-Z0-9]{8,16})/i)?.[1] || p.match(/\/gp\/product\/([A-Z0-9]{8,16})/i)?.[1] || p.match(/\/exec\/obidos\/ASIN\/([A-Z0-9]{8,16})/i)?.[1] || p.match(/\/o\/ASIN\/([A-Z0-9]{8,16})/i)?.[1] || url.searchParams.get("ASIN") || url.searchParams.get("ASIN.0") || (() => {
  ...
  function cleanURL() {
    ...
    try {
      ...
      let c = 0;
      for (const p of TRACK) if (url.searchParams.has(p)) {
      ...
      const step = () => {
        ...
        for (; i < end; i++) {
          ...
          try {
            ...
            let m = 0;
            if (u.href.includes("/ref=")) {
              a.href = h.replace("/ref=", "?ref=");
              m = 1;
            }
            for (const p of TRACK) if (u.searchParams.has(p)) {
  ...
  const forceGPU = (() => {
    ...
    return () => {
      document.querySelectorAll('video:not([data-wp-gpu]),canvas:not([data-wp-gpu]),img[loading="eager"]:not([data-wp-gpu])').forEach((el) => {
  ...
  function restoreScripts() {
    document.querySelectorAll('script[type="text/wp-blocked"][data-wp-id]').forEach((s) => {
      ...
      if (!src) return;
      const danger = src.startsWith("javascript:") || src.startsWith("data:") || src.startsWith("vbscript:") || src.startsWith("//") || src.includes("<") || src.includes(">") || src.includes('"') || src.includes("'");
      if (danger) {
