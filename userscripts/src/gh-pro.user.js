...
(() => {
	...
	function initEditorSettings() {
		...
		const applySettings = (cfg) => {
			...
			// Check context
			if (
				location.href.match(/^https?:\/\/github.com\/[^/]*\/[^/]*\/new\/.*/u)
			) {
				// New file
	...
	function initFileSizeViewer() {
		...
		async function fetchFileSize(apiUrl) {
			...
			try {
				...
				if (data && !Array.isArray(data) && data.type === "file") {
					...
					const { size, fileCount } = await calculateFolderSize(
						apiUrl,
						headers,
					);
					return size > 0
		...
		async function displayFileSizes() {
			try {
				...
				if (!tableBody) return;
				const links = tableBody.querySelectorAll(
				...
				console.log(
					"[GH-Tweaks] Processing",
					unprocessedLinks.length,
					"file/folder links",
				);
