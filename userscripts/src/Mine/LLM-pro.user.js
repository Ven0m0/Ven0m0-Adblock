...
// --- Intercept ChatGPT fetch for conversation reducing ---
if(isCGPT){
 const INIT_MSG=10,MAX_ITER=800,rx=/^https:\/\/chatgpt\.com\/backend-api\/conversation\/[a-f0-9\-]{36}$/i;
 ...
 window.fetch=function(input,init){
  ...
  return ogFetch.apply(this,arguments).then(async r=>{
   try{
    ...
    const nm=[],seen=new Set();
    let nid=d.current_node,nv=0,iterations=0;
    while(iterations++<MAX_ITER){
     if(!nid||seen.has(nid))break;
     const m=d.mapping[nid];if(!m)break;seen.add(nid);
     if(m.id==="client-created-root"){nm.push(m);break;}
     const cids=[...m.children];
     while(cids.length&&iterations<MAX_ITER){
      iterations++;
      const cid=cids.pop(),c=d.mapping[cid];
      if(!c||seen.has(cid))continue;
      seen.add(cid);nm.push(c);cids.push(...c.children);
     }
     if(nv<INIT_MSG&&d.mapping[m.parent])nm.push(m);
     else{nm.push({...m,parent:"client-created-root"});nm.push({id:"client-created-root",message:null,parent:null,children:[m.id]});break;}
     nid=m.parent;nv++;
    }
...
// --- Universal width/layout adjustment (all platforms) ---
const runReady=(sel,cb)=>{
 let n=0,maxAttempts=25;
 const t=()=>{
  const e=document.querySelector(sel);
  if(e)cb(e);else if(++n<maxAttempts)setTimeout(t,250*1.05**n);
 };
 t();
};
...
if(isCGPT){
 ...
 const gf=()=>{
  ...
  if(!e)return[];const l1=e.parentElement,l2=l1.parentElement;
  return [l1,l2];
 };
 runReady('div[data-is-streaming]',()=>{applyW(gf);observeW(gf);});
}

// --- ChatGPT DOM cleanup and auto-continue ---
if(isCGPT){
 const MAX_MSG=15;
 const cleanup=()=>{
 ...
 const intervals=[{delay:8e3,interval:1500},{delay:12e3,interval:2500},{delay:8e3,interval:4000}];
 let currentInterval=null,currentTimeout=null;
 const schedInt=idx=>{
 ...
 const getSubmit=()=>submitCache||(submitCache=document.querySelector('button[data-testid$="send-button"]'));
 ...
 const getContBtn=()=>{
  ...
  for(const b of buttons)if(b.textContent?.includes('Continue'))return b;
  return null;
 };
 const getRegenBtn=()=>{
     ...
     while(true){
      ...
      if(rb&&!getTx()){
       if(retries<2){
        ...
        rb.click();retries++;lastRetry=Date.now();continue;
       }else break;
      }
      first=false;break;
     }
