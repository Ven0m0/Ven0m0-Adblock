...
(function(){"use strict";
  ...
  (()=>{
    ...
    if(win?.indexedDB?.constructor?.name==="IDBFactory"){
      ...
      const cleanup=()=>{
        ...
        for(const req of openDBs)try{req.result?.close();}catch(_e){}
        openDBs.clear();
        // eslint-disable-next-line no-empty
        for(const [db] of closedDBs)try{db&&db.close();}catch(_e){}
        closedDBs.clear();
      };
      const dbCloseMap=new WeakMap();
      const scheduleClose=(db,_name)=>{
  ...
  if(CFG.cpu.rafDecimation){
    ...
    const processQueue=()=>{
      ...
      if(now>=nextFrm){
        ...
        // eslint-disable-next-line no-empty
        for(let i=0;i<bs;i++)try{cbs[i](now);}catch(_e){}
        // eslint-disable-next-line no-empty
        for(let i=bs;i<cbs.length;i++)origRAF(()=>{try{cbs[i](now);}catch(_e){}});
      }
  ...
  if(CFG.quality.enabled){
    const unwrapElement=el=>el&&el.wrappedJSObject?el.wrappedJSObject:el;
    ...
    const setResolution=(ytPlayer,resolutions)=>{
      ...
      const shouldPremium=CFG.quality.preferPremium&&[...ytPlayer.getAvailableQualityData()].some(
        ...
        try{
          ...
          if(settingsButton){
            ...
            const qualityMenuButton=document.evaluate(
              ...
              ytPlayer,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null
            ).singleNodeValue;
      ...
      if(CFG.quality.useAPI&&ytPlayer.getPlaybackQuality===undefined){
        ...
        if(curVid!==recentVideo||framerateUpdate){
          ...
          if(!storedQuality||storedQuality.indexOf(CFG.quality.targetRes)===-1){
            const tc=Date.now(),te=tc+2592000000;
            localStorage.setItem("yt-player-quality",
    ...
    window.addEventListener("yt-navigate-finish",initQuality,!0);
  ...
  (async()=>{
    ...
    try{
      const settingsSaved=await getStoredValue("settingsSaved",!1);
      if(CFG.quality.overwriteStoredSettings||!settingsSaved){
        ...
        await setStoredValue("settingsSaved",!0);
        log("Settings saved");
      }else{
